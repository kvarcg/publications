...
uniform samplerCube sampler_shadow_map;
uniform vec3 light_color;
uniform vec3 light_position;
uniform int light_has_attenuation;
...
{
	vec3 p_direction = pwcs - light_position;
	//p_direction = normalize(p_direction);
	
	// sample shadow map
	float shadow_map_z = texture(sampler_shadow_map, p_direction).r;

	vec3 p_to_light = pwcs - light_position;
	p_to_light = abs(p_to_light);
	float p_z = max(p_to_light.x, max(p_to_light.y, p_to_light.z));
...
/*float shadow_pcf_3x3(vec3 light_space_xyz)
{
	...
	// [-1, +1]
	shadow_map_z = texture(sampler_shadow_map, light_space_xyz.xy + vec2(- shadow_map_step, + shadow_map_step)).r;
	sum += clamp(-sign(light_space_xyz.z - uniform_constant_bias - shadow_map_z), 0.0, 1.0);
	// [0, +1]
	shadow_map_z = texture(sampler_shadow_map, light_space_xyz.xy + vec2(0.0, + shadow_map_step)).r;
	sum += clamp(-sign(light_space_xyz.z - uniform_constant_bias - shadow_map_z), 0.0, 1.0);
	// [+1, -1]
	shadow_map_z = texture(sampler_shadow_map, light_space_xyz.xy + vec2(+ shadow_map_step, + shadow_map_step)).r;
	sum += clamp(-sign(light_space_xyz.z - uniform_constant_bias - shadow_map_z), 0.0, 1.0);
	// [-1, 0]
	shadow_map_z = texture(sampler_shadow_map, light_space_xyz.xy + vec2(- shadow_map_step, 0.0)).r;
	sum += clamp(-sign(light_space_xyz.z - uniform_constant_bias - shadow_map_z), 0.0, 1.0);
	// [+1, 0]
	shadow_map_z = texture(sampler_shadow_map, light_space_xyz.xy + vec2(+ shadow_map_step, 0.0)).r;
	sum += clamp(-sign(light_space_xyz.z - uniform_constant_bias - shadow_map_z), 0.0, 1.0);
...
/*float shadow_pcf_gaussian(vec3 pwcs)
{
	...
	// sample shadow map
	float shadow_map_z = texture(sampler_shadow_map, light_to_p).r;
