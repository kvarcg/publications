...
{
	vec3 p_to_light = light_position - pwcs;
...
//	float radius = 0.5 + 30.5 * abs((light_space_xyz_ecs.z - light_space_xyz_sample_ecs.z) / light_space_xyz_ecs.z);
	...
	vec2 offset_jittering = 0.4 * texture2D(sampler_noise, 220 * (plcs_direction.xy-vec2(0.5,0.5))).xy;
...
float shadow(vec3 pecs)
{
	...
	vec3 plcs = light_position - pwcs.xyz;
