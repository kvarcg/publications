...
uniform float light_attenuation_far_range;
...
float shadow_pcf_gaussian(vec3 light_space_xyz, vec4 light_space_xyz_ecs)
{
	float shadow_map_z = texture2D(sampler_shadow_map, light_space_xyz.xy).r;
	...
	//vec4 light_space_xyz_ecs = vec4(2.0 * light_space_xyz - 1.0, 1.0);
	...
	for (int i = 0; i < 16; i++)
	{
		...
		//vec4 light_space_xyz_sample_ecs =  vec4(2.0 * light_space_xyz.xy - 1.0, 2.0 * shadow_map_z - 1.0, 1.0);
		//light_space_xyz_sample_ecs /= light_space_xyz_sample_ecs.w;

		sum_radius += 10 * abs((light_space_xyz_ecs.z - shadow_map_z) / light_space_xyz_ecs.z);
	}
	sum_radius /= 16.0;

	sum_radius *= light_size;
...
float shadow(vec3 pecs)
{
	...
	vec4 plcs = uniform_light_view * pwcs;

	//if( plcs.z > 0.0 ) return 0.0;
	
	float plcs_length = length(plcs.xyz);
	vec4 pnlcs = plcs/ plcs_length;
	pnlcs.x /= 1.0 - pnlcs.z;
	pnlcs.y /= 1.0 - pnlcs.z;
	pnlcs.z = (plcs_length - uniform_light_near_range) / (uniform_light_far_range - uniform_light_near_range);
	pnlcs.w = 1.0;
	pnlcs.xy = (pnlcs.xy + 1) * 0.5;
	
	// check that we are inside light clipping frustum
	//if (pnlcs.x < 0.0) return 0.0; if (pnlcs.y < 0.0) return 0.0; if (pnlcs.x > 1.0) return 0.0; if (pnlcs.y > 1.0) return 0.0;
	if ((clamp(pnlcs.xy, vec2(0,0), vec2(1,1)) - pnlcs.xy) != vec2(0,0)) return 0.0;

	// set scale of shadow map value to [-1,1] or
	// set scale of light space z vaule to [0, 1]
	pnlcs.z = (pnlcs.z + 1) * 0.5;

	initSlopeBias(pnlcs.xyz);

	float shadowFactor = 1;
	
	shadowFactor = shadow_nearest(pnlcs.xyz);

	//shadowFactor = shadow_pcf_3x3(plcs.xyz);
	
	//shadowFactor = shadow_pcf_2x2_bilinear_interpolation(plcs.xyz);
	
	shadowFactor = shadow_pcf_gaussian(pnlcs.xyz, plcs);
...
void main(void)
{
	...
	vec3 dirColor = (diff_brdf + specular) * irradiance;

	//vec3 dirColor = (specular) * irradiance;
