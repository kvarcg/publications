...
//                                                    //
//----------------------------------------------------//
// Multi-view AO, based on the Alchemy AO method by Morgan McGuire
...
void main(void)
{
...
#if (NUM_VIEWS>3)
	// VIEW 4
	// MVAO calculation for view 3 (same as above)
	view_index = 3;
	view_dir = normalize(uniform_view_position[view_index] - pwcs);
	view_normal_bias = max(0.0, dot(normal_unclamped_wcs, view_dir));
	int view4_samples = init_samples;
	calculate_view(view_index, 0, view4_samples, pwcs, rotation_scale, vec_u, vec_v, normal_unclamped_wcs, view_occlusion, view_bent_normal, view_distance_bias);
	view_distance_bias *= distance_bias_weight;
	view_normal_bias *=  normal_bias_weight;
	view4_occlusion = view_occlusion;
	view4_weights = view_normal_bias + view_distance_bias;
	view4_bent_normal = view_bent_normal;
#endif
	...
	int num_samples4 = int(floor(1+(uniform_num_samples-init_samples)*(0.5*view4_occlusion+0.5)*view4_weights/(view1_weights+view2_weights+view4_weights)));
	adaptive_index = 2;
	calculate_view(adaptive_index, init_samples, init_samples+num_samples4, pwcs, rotation_scale, vec_u, vec_v, normal_unclamped_wcs, adaptive_occlusion, adaptive_bent_normal, view_distance_bias);
	view_dir = normalize(uniform_view_position[adaptive_index] - pwcs);
	view_normal_bias = max(0.0, dot(normal_unclamped_wcs, view_dir)) * normal_bias_weight;
	view_distance_bias *= distance_bias_weight;
	adaptive_weights = view_normal_bias + view_distance_bias;
	view4_occlusion = (view4_occlusion*init_samples+adaptive_occlusion*num_samples4)/(init_samples+num_samples4);
	view4_weights = (view4_weights*init_samples + adaptive_weights*num_samples4)/(init_samples+num_samples4);
	view4_bent_normal += adaptive_bent_normal;
#endif
#endif	
	
	// calculate total occlusion based on partial obscurance results and weights
	//total_occlusion = view1_occlusion * view1_weights + view2_occlusion * view2_weights + view3_occlusion * view3_weights + view4_occlusion * view4_weights;
	//total_occlusion /= view1_weights + view2_weights + view3_weights + view4_weights;

	total_occlusion = view1_occlusion * view1_weights + view2_occlusion * view2_weights + view3_occlusion * view3_weights;// + view4_occlusion * view4_weights;
	total_occlusion /= view1_weights + view2_weights + view3_weights;// + view4_weights;

	//total_occlusion = view3_occlusion;
	...
	total_bent_normal = view1_bent_normal * view1_weights + view2_bent_normal * view2_weights + view3_bent_normal * view3_weights + view4_bent_normal * view4_weights;
	...
	// retrieve normal information from our normal buffer
	vec3 normal_unclamped_ecs = vec4(vec4(normal_decode_spheremap1(texture2D(sampler_normal, TexCoord.xy).xy), 0.0)).rgb;
	normal_unclamped_ecs = normalize(normal_unclamped_ecs);

	//// get the fragment's world space position
	//vec3 pecs = vec4(vec4(reconstruct_position_from_depth(), 1)).xyz;
//
	//view_dir = normalize((uniform_view[0] * vec4(uniform_view_position[0],1)).xyz - pecs);
	//
	//vec3 N = normalize(cross(dFdy(pecs), dFdx(pecs))); // N is the world normal
//
	//float d = dot(view_dir, normal_unclamped_ecs);
	//if (d >= 0) total_bent_normal = total_bent_normal;//vec3(0,1,0);
	//else if (d < 0) total_bent_normal = total_bent_normal;//vec3(1,0,0);
	//else {
//
	//total_bent_normal = N;
	//}
//
	//if (normal_unclamped_ecs.x == 0 && normal_unclamped_ecs.y == 0 && normal_unclamped_ecs.z == 0) {out_color = vec4(1,1,0,1); return;}
	//if (view_dir.x == 0 && view_dir.y == 0 && view_dir.z == 0) {out_color = vec4(1,0,1,1); return;}
