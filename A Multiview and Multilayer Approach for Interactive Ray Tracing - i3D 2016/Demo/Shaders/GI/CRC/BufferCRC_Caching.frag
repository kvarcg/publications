...
void main()
{
	...
	{
		// get a random RSM sample in uv coordinates and project it in the RSM depth buffers
		vec2 seed = pos_wcs.xy * 11 + pos_wcs.yz * 17 + l_uv.xy * 17 + i/float(uniform_num_samples);
		vec2 r = rand2n(seed);
		//vec2 uv = uv_c + r * sp;
	...
	// scale by the resolution which was avoided in the RSM step for precision issues
	float tex_res		= float(textureSize(sampler_shadow_color,0).x);
