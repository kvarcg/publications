...
// 3. traceScreenSpaceRay_abuffer_cube - trace a ray hierarchically in screen-space for a particular view
...
// and min-max hierarchical screen-space tracing
//
...
// - in case the iteration has reached the lowest lod level (denoted by the uniform_ab_mipmap variable), check the A-buffer for an actual intersection
...
int traceScreenSpaceRay_abuffer_cube
	...
	float pixel_offset = iteration == 0 ? 0.5 + jitter : jitter;
	...
	while (P.x * stepDirection <= end && rayZMax < 0 && lod+1 > 0)
	{
		...
		layer = ray_hit_a_buffer_search(ivec2(f_viewport_c), rayZMin, rayZMax, signdz, divstep, lod, cubeindex
		#ifdef LAYER_VISUALIZATION
				, depth_layer
		#endif
				);
		...
		if (layer == invalid_result)
		{
			...
			lod = min(uniform_lod_max+1, lod+2);
