...
// for linear screen-space ray tracing:
...
// Steps: 
// 1. Perform early skip based on the pixel's depth extents
...
int ray_hit_a_buffer_search(ivec2 coords, float minZ, float maxZ, int increment, int cubemapindex 
...
)
{
	...
	for(int i=b0; i <= b1 && index_max <= 0 && layers < NUM_LAYERS; i++)
	{
...
#else // multi-layer bi-directional implementation			
