...
void drawViewport(vec2 P0, vec2 P1, int lod)
{
	if (lod == 0) return;
	vec2 start = P0;
	vec2 end = P1;
	vec4 c = vec4(0.05, 0.05, 0, 1);
	for (float i = start.x; i < end.x; ++i)
	{
		imageStore(image_preview, ivec3(i, start.y, 1), c);
		imageStore(image_preview, ivec3(i, end.y, 1), c);
	}
	
	for (float i = start.y; i < end.y; ++i)
	{
		imageStore(image_preview, ivec3(start.x, i, 1), c);
		imageStore(image_preview, ivec3(end.x, i, 1), c);
	}
}

vec4 traceTest_hiz_sampling(ivec2 size, int lod_max, bool viewport_render, vec4 hit_color)
{
	...
	float clipped_length = min(length_to_near, range);
	...
	float rayZMin = (Q.z) / (k - dk * half_pixel_offset);
	float rayZMax = (Q.z) / (k + dk * half_pixel_offset);
	...
	float	divstep = 1.0;
	vec2 hitPixelFuture;
	ivec2	lod_coords, lod_coords_future;
	vec4	f_viewport, f_viewport_future;
	float	lod_alpha, lod_alpha_future, lod_alpha_past;
	vec2 _P0 = permute ? P0.yx : P0; 
	vec2 _P1 = permute ? P1.yx : P1;

	vec4  depths;
	float depth_near, depth_far;
	while (P.x * stepDirection <= end && rayZMax < 0 && lod > -1)
	{
		// verification check
		...
		lod_mod2 = lod % 2;
		if (lod_mod2 == 0)
			depths = texelFetch(tex_depth_bounds, ivec2(hitPixel*divstep), lod/2);
		depth_near =   -depths[(lod_mod2 *2)  ];
		depth_far  =	depths[(lod_mod2 *2)+1];

		layer = ray_hit_a_buffer_search(ivec2(hitPixel), rayZMin, rayZMax, signdz, depth_near, depth_far, lod);
		...
		if (layer == -1)
		{
			...
			f_viewport.zw = f_viewport.xy  + step;
			...
			Q = mix(Q, Q1, lod_alpha); 
			k = mix(k, k1, lod_alpha); 

			P += dP * half_pixel_offset;
			Q += dQ * half_pixel_offset;
			k += dk * half_pixel_offset;

			// reset lod - recompute pixel step
			lod = min(lod_max, lod+2);

			//if (isDebugFragment() && viewport_render)
				//drawViewport(f_viewport.xy, f_viewport.zw, lod);
		}
		...
		divstep = 1.0 / step;
		hitPixelFuture = permute ? P.yx : P;
		lod_coords_future = ivec2(floor(hitPixelFuture * divstep));

		f_viewport_future.xy = lod_coords_future   << lod;
		f_viewport_future.zw = f_viewport_future.xy + vec2(step);

		lod_alpha_past = clipViewportLod(hitPixelFuture, _P0, f_viewport_future);
		float k_past = mix(k, k0, lod_alpha_past);
		vec3  Q_past = mix(Q, Q0, lod_alpha_past);
		rayZMin = (Q_past.z) / (k_past);

		lod_alpha_future = clipViewportLod(hitPixelFuture, _P1, f_viewport_future);		
		float k_future = mix(k, k1, lod_alpha_future);
		vec3  Q_future = mix(Q, Q1, lod_alpha_future);
		rayZMax  = (Q_future.z) / (k_future);
	}

	// just write the ray points to the ray texture
	if (isDebugFragment())
	{
		imageStore(image_preview, ivec3(start_points.xy, 1), vec4(1, 1, 1, 1));
		imageStore(image_preview, ivec3(start_points.zw, 1), vec4(0, 1, 0, 1));
	}

	// result value
	vec4 fetched_value = (layer > -1) ? FetchResult(ivec3(hitPixel, layer)) : vec4(0);
	return fetched_value;
}

vec4 traceTest_linear_sampling(ivec2 size)
{
	buffer_size = imageSize(image_test).xy;
	...
	for (; P.x * stepDirection <= end && layer < 0 && rayZMax < 0; P += dP, Q += dQ, k += dk, stepCount++)
	{
		...
		vec2  depths = texelFetch(tex_depth_bounds, ivec2(hitPixel), 0).rg;
		depth_near   = -depths.r;
		depth_far    = depths.g;

		layer = ray_hit_a_buffer_search(ivec2(hitPixel), rayZMin, rayZMax, signdz, depth_near, depth_far, 0);
	}
	// just write the ray points to the ray texture
	if (isDebugFragment())
	{
