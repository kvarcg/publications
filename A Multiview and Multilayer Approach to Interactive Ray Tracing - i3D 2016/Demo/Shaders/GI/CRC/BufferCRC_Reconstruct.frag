...
void main(void)
{
	...
	vec3 normal_ecs = normal_decode_spheremap1(texture(sampler_normal,TexCoord.st).xy);
	...
	float scatter = (1.0 - ks.x)*(1.0-ks.z);
	//gi_diffuse_color.xyz *= 1 * kd / 3.1459;
	gi_diffuse_color.xyz *= scatter * kd / 3.1459;
	...
	occlusion = texture(sampler_ssao, TexCoord.st).r;
	if (uniform_use_ao > 0) 
	{
		gi_diffuse_color.xyz *= occlusion;
	}
