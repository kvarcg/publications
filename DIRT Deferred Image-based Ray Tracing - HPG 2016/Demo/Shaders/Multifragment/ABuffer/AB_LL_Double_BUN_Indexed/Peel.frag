...
// Decoupled double linked-list with buckets fragment implementation of Peel stage
...
uvec2 imageLoadHT (const int  b)			{ return imageLoad(image_head, ivec3(gl_FragCoord.xy, b)).xy;}
void imageStoreHT (const int  b, uvec2 val)	{ imageStore(image_head, ivec3(gl_FragCoord.xy, b), uvec4(val, 0u, 0u));}
#else
