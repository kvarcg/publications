...
// Decoupled double linked-list fragment implementation of Peel stage
...
uvec2 imageLoadHT ()			{ return imageLoad(image_pointers, ivec2(gl_FragCoord.xy)).xy;}
void imageStoreHT (uvec2 val)	{ imageStore(image_pointers, ivec2(gl_FragCoord.xy), uvec4(val, 0u, 0u));}
#else
...
void main(void)
{
	...
	while (keepWaiting)
	{
		...
		{
			uvec2 currentHT = imageLoadHT();
			nodes[index].next = currentHT.x;
			currentHT.x = index;
			imageStoreHT(currentHT);
